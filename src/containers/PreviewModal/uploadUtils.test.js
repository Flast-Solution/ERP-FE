...
describe('upload asset URLs', () => {
  ...
  it.each([
    ['http://localhost:3000/api/upload/folder/view?filename=document-template%2Flogo.png', '/api/upload/folder/view/document-template/logo.png'],
    ['https://localhost:3000/api/upload/folder/view?filename=image.png#preview', '/api/upload/folder/view/image.png#preview'],
    ['http://127.0.0.1:3000/api/upload/folder/view?filename=image.png', '/api/upload/folder/view/image.png'],
    ['http://[::1]:3000/api/upload/folder/view?filename=image.png', '/api/upload/folder/view/image.png'],
  ...
  it.each([
    ...
    ['/api/upload/folder/view/logo.png', '/api/upload/folder/view/logo.png'],
  ...
  it('creates a portable URL when uploading on the development server', () => {
    ...
    expect(resolveUploadUrl({ fileName: 'document-template/image/photo 1.png' })).toBe(
      '/api/upload/folder/view/document-template/image/photo-1.png',
    )
  })
  ...
  it('keeps an explicit remote API origin when the deployment uses one', () => {
    ...
    expect(resolveUploadUrl('document-template/logo/logo.png')).toBe(
      'https://api.example.com/api/upload/folder/view/document-template/logo/logo.png',
    )
  })
  ...
  it('adds the deployed API origin to an API-relative image URL', () => {
    ...
    expect(resolveRuntimeAssetUrl('/api/upload/folder/view?filename=document-template%2Flogo.png')).toBe(
      'http://157.10.199.138:9080/api/upload/folder/view/document-template/logo.png',
    )
  })
  ...
  it('moves a persisted localhost image URL to the deployed API origin', () => {
    ...
    expect(resolveRuntimeAssetUrl('http://localhost:3000/api/upload/folder/view?filename=logo.png')).toBe(
      'https://api.example.com/api/upload/folder/view/logo.png',
    )
  })
  ...
  it('migrates an old remote query URL to the path format', () => {
    ...
    )).toBe('http://157.10.199.138:9080/api/upload/folder/view/test/my-image.png')
  })
})
