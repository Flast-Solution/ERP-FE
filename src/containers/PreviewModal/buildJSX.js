...
function buildUploadHelper(fields) {
  ...
  return [
    ...
    `    const markerIndex = matchedMarker ? parsedUrl.pathname.toLowerCase().indexOf(matchedMarker.toLowerCase()) : -1`,
    ...
    `    return absolute ? parsedUrl.toString() : parsedUrl.pathname + parsedUrl.search + parsedUrl.hash`,
    `  } catch { return url }`,
    `}`,
    ...
    `  const baseUrl = String(axios.defaults.baseURL || '/api').replace(/\\/$/, '')`,
    `  return resolveRuntimeAssetUrl(baseUrl + '/erp/folder/view/' + normalizeUploadPath(filename))`,
    `}`,
    ``,
