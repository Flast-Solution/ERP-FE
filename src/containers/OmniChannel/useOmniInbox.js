...
export const useOmniInbox = () => {
  ...
  const openConversation = useCallback(
    async (conversationId) => {
      ...
      if (previous) socketRef.current?.unsubscribe(previous)
      s.openConversation(conversationId)
      socketRef.current?.subscribe(conversationId)

      /* Đồng bộ URL để sale gửi link cho nhau, KHÔNG dùng router
         để tránh remount cả ba cột */
      ...
      if (last) socketRef.current?.markRead(conversationId, last.id)
  ...
  const notifyTyping = useCallback(() => {
    ...
    if (!id) return
    const now = Date.now()
    if (now - typingRef.current < 2000) return
