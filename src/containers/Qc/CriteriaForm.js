...
const CriteriaForm = ({ data, closeModal }) => {
    const [form] = Form.useForm(); // Tạo form instance mới
    ...
    const isBooleanType = evaluationType === 1;  // BOOLEAN
    const isScoreType = evaluationType === 2;    // SCORE
    const isQuantityType = evaluationType === 3; // QUANTITY
    const isTextType = evaluationType === 4;     // TEXT
    ...
    return (
        <RestEditModal
            ...
            record={data}
            closeModal={closeModal}
            form={form}  // Truyền form vào RestEditModal
        >
            <Row gutter={16} style={{ marginTop: 20 }}>
                ...
                <Col md={12} xs={24}>
                    <FormSelect
                        ...
                        titleProp="name"
                        onChange={() => {
                            // Reset các field khi đổi loại đánh giá
                            form.setFieldsValue({
                                targetMin: undefined,
                                targetMax: undefined,
