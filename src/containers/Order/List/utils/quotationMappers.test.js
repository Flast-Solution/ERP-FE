...
describe('quotation quoteConfig mapping', () => {
  ...
  it('stores imported marked-sheet tables in quoteConfig and restores them', () => {
    const order = makeOrder()
    const data = createQuotationData(order, template)
    data.sheetTables = { packingList: {
      columns: [{ key: 'rollNo', label: 'Roll No.', type: 'text' }],
      rows: [{ rollNo: 'ROLL-1' }],
      totals: {},
    } }
    const payload = buildQuotationPayload(data, template, order)
    expect(payload.quoteConfig.sheetTables).toEqual(data.sheetTables)
    const reopened = createQuotationData({ ...order, quoteConfig: payload.quoteConfig }, template)
    expect(reopened.sheetTables).toEqual(data.sheetTables)
  })
  ...
  it('round-trips manual fields in nested templates without adding them to business data', () => {
    ...
    expect(payload.quoteConfig.details).toEqual({
      34079: { manualValues: { moq: '1000', mcq: '200', date: '20-10-2026' } },
      34080: { manualValues: { moq: '123', mcq: '333', date: '22-11-2026' } },
    })
    expect(payload.details.every(detail => !Object.prototype.hasOwnProperty.call(detail, 'quoteConfig'))).toBe(true)
    expect(payload.details).toEqual(order.details.map(({ quoteConfig, ...detail }) => detail))
  ...
  it.each(['', null, 0, false])('preserves an explicit manual value of %p over legacy data', value => {
    ...
    expect(payload.quoteConfig.details['34079'].manualValues.moq).toBe(value)
  })
  ...
  it.each([null, '{invalid', [], 'null'])('falls back to legacy fields with absent/invalid config %p', quoteConfig => {
    ...
    expect(buildQuotationPayload(data, template, order).quoteConfig.details['34079'].manualValues.moq).toBe('1000')
  })
  ...
  it('preserves other config and fields removed from the template, including JSON string responses', () => {
    ...
    expect(payload.quoteConfig.details['34079']).toEqual({
      otherSetting: true,
    ...
    expect(saved.quoteConfig.details['34079']).toEqual(payload.quoteConfig.details['34079'])
    expect(saved.details.every(detail => !Object.prototype.hasOwnProperty.call(detail, 'quoteConfig'))).toBe(true)
  })
  ...
  it('handles nested bindings and JSON values without mutating the API snapshot', () => {
    ...
    const payload = buildQuotationPayload(data, template, order)
    expect(payload.quoteConfig.details['34079'].manualValues['terms.delivery']).toEqual({ days: [1, 2, 3] })
    expect(payload.details[0].terms).toBeUndefined()
  })

  it('merges partial server responses without dropping other saved invoice templates', () => {
    const order = makeOrder()
    const payload = buildQuotationPayload(createQuotationData(order, template), template, order)
    payload.quoteConfig = { invoiceTemplates: {
      invoiceA: { manualValues: { note: 'A' } },
      invoiceB: { sheetTables: { packing: { rows: [{ no: 1 }] } } },
    } }
    const saved = mergeSavedQuotationOrder(order, payload, {
      quoteConfig: { invoiceTemplates: { invoiceA: { serverSetting: true } } },
    })
    expect(saved.quoteConfig.invoiceTemplates).toEqual({
      invoiceA: { manualValues: { note: 'A' }, serverSetting: true },
      invoiceB: { sheetTables: { packing: { rows: [{ no: 1 }] } } },
    })
  })
  ...
  it('does not overwrite native price when a template makes it manually editable', () => {
    ...
    expect(payload.quoteConfig.details['34079'].manualValues.price).toBe(250000)
